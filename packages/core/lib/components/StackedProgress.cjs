...
class StackedProgress extends ProgressBase.ProgressBase {
  constructor() {
    super(...arguments);
    this.layers = [];
  }
  ...
  render() {
    ...
    this.wrapper = helpers.createElement("div", "ld-progress-stacked", this.container);
    this.wrapper.style.position = "relative";
    this.wrapper.style.width = helpers.toPx(width);
    this.wrapper.style.height = helpers.toPx(height + layerOffset * layerCount + depth);
    ...
    for (let i = 0; i < layerCount; i++) {
      const layer = helpers.createElement("div", "ld-progress-stacked__layer", this.wrapper);
      layer.style.position = "absolute";
      layer.style.width = helpers.toPx(width);
      layer.style.height = helpers.toPx(height);
      ...
      const bg = helpers.createElement("div", "ld-progress-stacked__bg", layer);
      bg.style.width = "100%";
      bg.style.height = "100%";
      bg.style.backgroundColor = "#e4e7ed";
      bg.style.opacity = String(0.3 + i * 0.1);
      const bar = helpers.createElement("div", "ld-progress-stacked__bar", layer);
...
exports.StackedProgress = StackedProgress;
//# sourceMappingURL=StackedProgress.cjs.map
