...
var DashboardProgress = require('./DashboardProgress.cjs');
...
class GaugeProgress extends DashboardProgress.DashboardProgress {
  constructor() {
    super(...arguments);
    this.numberElements = [];
  }
  ...
  renderNumbers() {
    ...
    for (let i = 0; i <= tickCount; i++) {
      ...
      const text = helpers.createSVGElement("text", {
        x,
        y,
        "text-anchor": "middle",
  ...
  updateNeedleStyle() {
    ...
    switch (needleType) {
      case "arrow":
        const arrowPath = helpers.createSVGElement("path", {
        ...
        break;
      case "triangle":
        const trianglePath = helpers.createSVGElement("path", {
          d: `M ${center} ${center - pointerLength}
              L ${center - 6} ${center}
              L ${center + 6} ${center} Z`,
          fill: "#f56c6c"
...
exports.GaugeProgress = GaugeProgress;
//# sourceMappingURL=GaugeProgress.cjs.map
