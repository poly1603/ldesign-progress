...
class SparklineProgress extends ProgressBase.ProgressBase {
  constructor() {
    super(...arguments);
    this.dataPoints = [];
  }
  ...
  createFillArea() {
    ...
    this.fillPath = helpers.createSVGElement("path", {
      d: pathData,
  ...
  createLine() {
    ...
    let pathData;
    if (smooth) {
      pathData = helpers.createBezierPath(points, 0.3);
    } else {
    ...
    this.dataPath = helpers.createSVGElement("path", {
      d: pathData,
      fill: "none",
  ...
  createDots() {
    ...
    points.forEach((point) => {
      const dot = helpers.createSVGElement("circle", {
        cx: point.x,
        cy: point.y,
        r: dotSize,
        fill: lineColor,
...
exports.SparklineProgress = SparklineProgress;
//# sourceMappingURL=SparklineProgress.cjs.map
