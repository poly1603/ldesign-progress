...
class WaveProgress extends ProgressBase.ProgressBase {
  constructor() {
    super(...arguments);
    this.waveOffset = 0;
  ...
  render() {
    ...
    this.wrapper = helpers.createElement("div", "ld-progress-wave", this.container);
    ...
    if (this.config.get("showText")) {
      this.textElement = helpers.createElement("div", "ld-progress-wave__text", this.wrapper);
  ...
  renderCanvas() {
    ...
    this.canvas = helpers.createElement("canvas", "ld-progress-wave__canvas", this.wrapper);
    this.canvas.width = widthNum;
    this.canvas.height = heightNum;
    this.canvas.style.position = "absolute";
    this.canvas.style.top = "0";
    this.canvas.style.left = "0";
    this.useOffscreen = helpers.supportsOffscreenCanvas();
    if (this.useOffscreen) {
      this.offscreenCanvas = new OffscreenCanvas(widthNum, heightNum);
      this.ctx = this.offscreenCanvas.getContext("2d");
    } else {
      this.ctx = MemoryManager.canvasContextCache.get(this.canvas, { alpha: true });
    }
  }
  renderSVG() {
    const width = this.config.get("width") ?? 200;
...
exports.WaveProgress = WaveProgress;
//# sourceMappingURL=WaveProgress.cjs.map
